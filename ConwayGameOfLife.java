...
public class ConwayGameOfLifeV2 {
    public static void main(String[] args) {
        ...
        int fieldWidth = fieldSize[0];
        int fieldHeight = fieldSize[1];

        // Set number of live cells to random number
        Random random = new Random();
        // I want there to be at least 3 live cells in the initial field
        int numberOfLiveCells = random.nextInt(3, (int) (fieldWidth * fieldHeight) / 2);
        System.out.println("numberOfLiveCells: " + numberOfLiveCells);

        int[][] field = initializeArray(fieldWidth, fieldHeight, numberOfLiveCells);
    ...
    public static int[][] randomlyMarkCellsLive(int[][] field, int numberOfLiveCells) {
        ...
        for (int j = 0; j < field.length; j++) {
            for (int k = 0; k < field[j].length && cellCounter < numberOfLiveCells; k++) {
                Random random = new Random();
